...
if(length(new.packages) > 0) install.packages(new.packages, lib = "/users3/fculce3c/jgcarvalho/R/x86_64-redhat-linux-gnu-library/3.6",  repos = "http://cran.r-project.org")
# load the packages
...
createParams <- function(NE, ratio, split, pool, seq, CW = NA, WC = NA, Mb = NA, Md = NA, site = NA, anc = NA, bT = NA, 
  ...
  # the different types of models have different numbers of parameters
  if (model == "noMig") {
    # set the number of parameters of interest for the noMig model
    nparams = 6 
    
  } else if (model == "simple2pops") {
    # set the number of parameters of interest for a simplified 2 populations model
    nparams = 10 
    
  } else if (model == "2pops") {
    # set the number of parameters of interest for the 2pops model
    nparams = 12
    
  } else if (model == "star") {
    # set the number of parameters of interest for the star model
    nparams = 16
    
  } else {
    # set the number of parameters of interest for the Single and Parallel models
    nparams = 20
  }
  ...
  # barrier against migration from crab to wave 
  bCW <- runif(n = 1, min = bCW[1], max = bCW[2])
  # barrier against migration from wave to crab 
  bWC <- runif(n = 1, min = bWC[1], max = bWC[2])
  # calculate the proportion of the genome without any barriers - unimpeded migration between the different ecotypes
  pMig <- (1-total)*(1-(bWC+bCW))
  # calculate the proportion of the genome without migration from crab to wave and add it to the matrix  
  pCW <- (1 - total)*bCW
  # calculate the proportion of the genome without migration from wave to crab and add it to the matrix
  pWC <- (1 - total)*bWC
  ...
  # Values are drawn as a ratio of the Ne size - a value of 2 means that the population is twice the size of the Ne 
  N3 <- round(runif(n = 1, min = ratio[1], max = ratio[2]), digits = 5)
  N4 <- round(runif(n = 1, min = ratio[1], max = ratio[2]), digits = 5)
  ...
  # Values are drawn as a ratio of the Ne size - a value of 2 means that the population is twice the size of the Ne 
  NA1 <- round(runif(n = 1, min = ratio[1], max = ratio[2]), digits = 5)
  NA2 <- round(runif(n = 1, min = ratio[1], max = ratio[2]), digits = 5)
  ...
  parameters <- c(Ne, N1, N2, N3, N4, NA1, NA2, Split, Dsplit, Pool_Error, Error, mCW, mWC, mCC, mWW, mAA, pMig, pCW, pWC, total)
  # add names to the columns
...
# CMD SIMPLE 2 POPS
# Create SCRM command line for a model with two populations and with migration
cmdSimple2pops <- function(parameters, nSites, nLoci, nDip, nPops, mutrate) {
...
# COMMAND LINE MIGRATION 2 POPS
# Create SCRM command line for a model with two populations and with migration
mig2pops <- function(parameters, nSites, nLoci, nDip, nPops, mutrate) {
  ...
  # get the proportion of loci with baseline migration rate - this is the genome wide migration rate
  pMb <- parameters[9]
  # and the proportion of loci with a migration rate that differs from the baseline migration rate
  pMd <- parameters[10]
  
  # get the baseline migration rate - between different ecotypes at the same site - this is the m value on the M = 4N0m formula
  # from the crab ecotype to the wave ecotype and vice versa
  Mb <-  parameters[7]
  # and the migration rate that differs from the baseline migration rate - also between different ecotypes at the same site 
  Md <-  parameters[8]
  
  # m <i> <j> <M>: Set the migration rate from population j to population i to M (looking forward in time) 
  # and REMEMBER that M = 4N0m
  # set the baseline migration rates to the scale of Nref - between different ecotypes at the same site
  Mb <- 4*Ne*Mb # -m 2 1 mig_CW and -m 1 2 mig_WC
  # also set the different migration rate to the scale of Nref
  Md <- 4*Ne*Md
  ...
  # use a multinomial distribution to get the number of loci simulated under each category
  lociTotal <- as.vector(rmultinom(n = 1, size = nLoci, prob = c(pMb, pMd)))
  
  # cheat code: pop1 - crab in site 1; pop2 - wave in site 1; pop3 - crab in site 2; pop4 - wave in site 2
  # create a command line for the loci a baseline migration rate - this is the genome wide migration rate between the ecotypes
  # set the basic elements for scrm - nhap: total number of haplotypes that are simulated at each locus and 
  # nrep: the number of independent loci that will be produced
  withMb <- paste(paste(nDip*2, collapse = " "), lociTotal[1], "-t",theta, "-I", paste(nPops), paste(n*2, collapse = " "), 
                  # set the size of the present day populations - n <i> <n> Set the size of population i to n*N0
                  "-n 1", N1, "-n 2", N2, 
                  # m <i> <j> <M>: Set the migration rate from population j to population i to M (looking forward in time)
                  # set the migration rate between different ecotypes inhabiting the same location
                  ...
                  # now, set the migration rate right before the split event to zero by using the switch:
                  # -eM <t> <M>: assume a symmetric migration rate of M/(npop-1) at time t.
                  "-eM", split - correctionTime, "0",
                  # add a split event -ej <t> <j> <i> in population i that creates population j (forwards in time)
                  # finally, set the size of the ancestral pop equal to the size of the reference population with:
                  # -eN <t> <n>: set the size of all populations to n*N0 at time t.
                  "-ej", split, "2 1 -eN", split + correctionTime, 1)
  
  # create a command line for the loci with a different migration rate - again between the ecotypes
  # set the basic elements for scrm - nhap: total number of haplotypes that are simulated at each locus and 
  # nrep: the number of independent loci that will be produced
  withMd <- paste(paste(nDip*2, collapse = " "), lociTotal[2], "-t",theta, "-I", paste(nPops), paste(n*2, collapse = " "), "0",
                  # set the size of the present day populations - n <i> <n> Set the size of population i to n*N0
                  "-n 1", N1, "-n 2", N2, 
                  # m <i> <j> <M>: Set the migration rate from population j to population i to M (looking forward in time)
                  # set the migration rate between different ecotypes inhabiting the same location
...
GetGenotypes <- function(Haplotypes, nDip) {
  ...
  # Sometimes scrm creates an output with zero polymorphic sites - this adds a single column with 0s across all individuals for those situations
...
# this function will compute the fraction of sites showing a fixed difference between two populations
...
exclusive <- function(ancestral, total, nPops) {
  ...
  if (nPops == 2) {
    ...
    keep <- colSums(seg) == 1
    # remove columns where populations are fixed for one allele or have no reads for that allele
    seg <- seg[, keep, drop = FALSE]
    # now by performing a sum over the rows we can obtain the number of sites, for each of the populations 
    # where that population is polymorphic while the others are not
    seg <- unname(rowSums(seg))
    # calculate the fraction of sites showing an exclusive polymorphism for each population
    gSeg <- seg / ncol(ancestral)
    # given that this produces 4 values, compute the mean to obtain the mean fraction of sites 
    # with an exclusive polymorphism for any population 
    gSeg <- mean(gSeg)
...
# update the poolABC function to include the majorSims function
poolABC <- function(parameters, model, nDip, nPops, size, nLoci, nSites, mutrate, mean, variance, 
  ...
  if(model == "noMig") {
    ...
    commands <- cmdPar(parameters, nSites, nLoci = extra, nDip, nPops, mutrate)
    
  } else if (model == "Single") {
    # create the command line for the single origin model
    commands <- cmdSin(parameters, nSites, nLoci = extra, nDip, nPops, mutrate)
  ...
  if(nLoci < target) {
    ...
    output <- list(NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA)
    # add names to the output
    names(output) = c("nPoly", "nFilter", "nLoci", "Sf", "Sx1", "Sx2", "SS", "Mean_Het1", "Mean_Het2", "SD_Het1", "SD_Het2", 
    ...
    Sf <- sapply(1:nLoci, function(locus) 
    ...
    # combine the previous list into a single matrix - where each column is a population
    Sx <- do.call(rbind, Sx)
    ...
    SS <- sapply(1:nLoci, function(locus) 
    ...
    # And the standard deviation
    SDFST <-apply(FST, 2, sd)
    # calculate the 5% and the 95% quantiles for the FST distribution
    FSTQ1 <- unname(quantile(FST, probs = 0.05))
    FSTQ2 <- unname(quantile(FST, probs = 0.95))
